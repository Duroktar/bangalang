...
import { BangaRuntime, IRuntimeBreakpoint, IRuntimeVariable, IRuntimeVariableType } from './bangaRuntime';
import { timeout } from "./timeout";

/**
 * This interface describes the banga-debug specific launch attributes
 * (which are not part of the Debug Adapter Protocol).
 * The schema for these attributes lives in the package.json of the banga-debug extension.
...
export class BangaDebugSession extends LoggingDebugSession {

	// we don't support multiple threads, so we can use a hardcoded ID for the default thread
	private static threadID = 1;

	// a Banga runtime (or debugger)
	private _runtime: BangaRuntime;
	...
	public constructor(fileAccessor: FileAccessor) {
		super("banga-debug.txt");

		// this debugger uses one-based lines and columns
		this.setDebuggerLinesStartAt1(true);
		this.setDebuggerColumnsStartAt1(true);

		this._runtime = new BangaRuntime(fileAccessor);

		// setup event handlers
		this._runtime.on('stopOnEntry', () => {
            // console.log('received: stopOnEntry');
			this.sendEvent(new StoppedEvent('entry', BangaDebugSession.threadID));
		});
		this._runtime.on('stopOnStep', () => {
			this.sendEvent(new StoppedEvent('step', BangaDebugSession.threadID));
		});
		this._runtime.on('stopOnBreakpoint', () => {
            // console.log('received: stopOnBreakpoint');
			this.sendEvent(new StoppedEvent('breakpoint', BangaDebugSession.threadID));
		});
		this._runtime.on('stopOnDataBreakpoint', () => {
			this.sendEvent(new StoppedEvent('data breakpoint', BangaDebugSession.threadID));
		});
		this._runtime.on('stopOnInstructionBreakpoint', () => {
			this.sendEvent(new StoppedEvent('instruction breakpoint', BangaDebugSession.threadID));
		});
		this._runtime.on('stopOnException', (exception) => {
            // console.log('received: stopOnException');
			if (exception) {
				this.sendEvent(new StoppedEvent(`exception(${exception})`, BangaDebugSession.threadID));
			} else {
				this.sendEvent(new StoppedEvent('exception', BangaDebugSession.threadID));
			}
		});
		this._runtime.on('breakpointValidated', (bp: IRuntimeBreakpoint) => {
            // console.log('received: breakpointValidated ->', bp);
	...
	protected async launchRequest(response: DebugProtocol.LaunchResponse, args: ILaunchRequestArguments) {
        // console.log('bangaDebug: launchRequest');
	...
	protected async setBreakPointsRequest(response: DebugProtocol.SetBreakpointsResponse, args: DebugProtocol.SetBreakpointsArguments): Promise<void> {
        // console.log('bangaDebug: setBreakPointsRequest');
	...
	protected threadsRequest(response: DebugProtocol.ThreadsResponse): void {
		...
		response.body = {
			threads: [
				new Thread(BangaDebugSession.threadID, "thread 1")
			]
		};
		this.sendResponse(response);
	}
	...
	protected stackTraceRequest(response: DebugProtocol.StackTraceResponse, args: DebugProtocol.StackTraceArguments): void {
        // console.log('bangaDebug: stackTraceRequest');
	...
	protected scopesRequest(response: DebugProtocol.ScopesResponse, args: DebugProtocol.ScopesArguments): void {
        // console.log('bangaDebug: scopesRequest');
		response.body = {
			scopes: [
	...
	protected async variablesRequest(response: DebugProtocol.VariablesResponse, args: DebugProtocol.VariablesArguments, request?: DebugProtocol.Request): Promise<void> {
        // console.log('bangaDebug: variablesRequest');
	...
	protected continueRequest(response: DebugProtocol.ContinueResponse, args: DebugProtocol.ContinueArguments): void {
        // console.log('bangaDebug: continueRequest');
	...
	protected async evaluateRequest(response: DebugProtocol.EvaluateResponse, args: DebugProtocol.EvaluateArguments): Promise<void> {
        // console.log('bangaDebug: evaluateRequest');
	...
	protected setExpressionRequest(response: DebugProtocol.SetExpressionResponse, args: DebugProtocol.SetExpressionArguments): void {
        // console.log('bangaDebug: setExpressionRequest');
		if (args.expression.startsWith('$')) {
		...
		if (Array.isArray(v.value)) {
			...
			switch (typeof v.value) {
				...
				case 'boolean':
					...
					break;
				default:
                    const name = v.value.constructor.name;
                    (<any>v.value).kind = name;
					dapVariable.value = this._runtime._typeChecker?.typeToString(v.value.type ?? (<any>v.value).decl.type);
					break;
			}
		}
	...
	private createSource(filePath: string): Source {
		return new Source(basename(filePath), this.convertDebuggerPathToClient(filePath), undefined, undefined, 'banga-adapter-data');
	}
}
